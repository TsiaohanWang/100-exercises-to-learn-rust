...
> [!TIP] 
> 如果将上面例子中的 `move` 去掉，程序会编译报错。这是因为闭包中捕获了环境中的 `v`，但由于闭包的捕获是默认不获取所有权的，因此这里实际上是 `v` 的不可变引用被传入了闭包，即 `&v` 进入了新线程中。但是编译器并不知道该线程会执行多久，如果这期间 `v` 在主线程被丢弃，那么新线程中的 `&v` 就会变为悬垂引用了。
>
