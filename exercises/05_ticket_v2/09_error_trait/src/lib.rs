...
use std::{fmt::{write, Display}, result};
use std::error::Error;
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        ...
        Err(TicketNewError::DescriptionError(err)) => Ticket { title: title.clone(), description: "Description not provided".to_string(), status: status.clone() }
    }
}
